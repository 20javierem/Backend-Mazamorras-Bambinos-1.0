...
async def create(schema: TransferBase, user=Depends(manager)):
    ...
    transfer.save()
    update_totals(transfer.source_id, transfer.productDaySale_id)
    update_totals(transfer.destiny_id, transfer.productDaySale_id)
    return transfer
...
async def update(id: int, schema: TransferUpdate, user=Depends(manager)):
    ...
    if sourceOld_id != transfer.source_id & sourceOld_id != transfer.destiny_id:
        update_totals(sourceOld_id, transfer.productDaySale_id)
    elif destinyOld_id != transfer.source_id & destinyOld_id != transfer.destiny_id:
        update_totals(destinyOld_id, transfer.productDaySale_id)
...
def update_totals(placeSale_id: int, productDaySale_id: int):
    productPlaceSale: ProductPlaceSale = product_place_sales.get_by_place_sale_and_product_day_sale(
