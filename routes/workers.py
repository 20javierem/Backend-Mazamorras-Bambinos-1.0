...
from models.worker import Worker, WorkerBase, WorkerUpdate, WorkerReadWithType
from routes.sessions import manager

apiWorkers = APIRouter()


@apiWorkers.get("/", response_model=list[WorkerReadWithType], status_code=status.HTTP_200_OK)
async def all(user=Depends(manager)):
    workers_list = await workers.all()
    return workers_list


@apiWorkers.post("/", response_model=WorkerReadWithType, status_code=status.HTTP_201_CREATED)
...
@apiWorkers.patch('/{id}', response_model=WorkerReadWithType, status_code=status.HTTP_202_ACCEPTED)
async def update(id: int, schema: WorkerUpdate, user=Depends(manager)):
    ...
    worker: WorkerReadWithType = await workers.save(worker)
    return worker
